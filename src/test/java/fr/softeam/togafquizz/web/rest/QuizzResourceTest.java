...
public class QuizzResourceTest {
	...
	private static final int NUMBER_OF_INITIAL_QUIZZ = 3;
	...
	public void setup() {
		...
		ReflectionTestUtils.setField(quizzResource, "quizzRepository",
				quizzRepository);
	...
	public void createQuizz() throws Exception {
		...
		assertThat(quizzRepository.findAll()).hasSize(NUMBER_OF_INITIAL_QUIZZ);

		// Create the Quizz
		restQuizzMockMvc.perform(
				post("/api/quizzs").contentType(TestUtil.APPLICATION_JSON_UTF8)
		...
		assertThat(quizzs).hasSize(NUMBER_OF_INITIAL_QUIZZ + 1);

		Quizz testQuizz = null;
		Iterator<Quizz> quizzIt = quizzs.iterator();

		while (quizzIt.hasNext()) {
			testQuizz = quizzIt.next();
		}

		assertThat(testQuizz.getLibelle()).isEqualTo(DEFAULT_LIBELLE);
		assertThat(testQuizz.getNumero()).isEqualTo(DEFAULT_NUMERO);
		assertThat(testQuizz.getDureeEnMinutes()).isEqualTo(
	...
	public void getQuizz() throws Exception {
		...
		// Get the quizz
		restQuizzMockMvc
				.perform(get("/api/quizzs/{id}", quizz.getId()))
				.andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON))
	...
	public void getNonExistingQuizz() throws Exception {
		// Get the quizz
		restQuizzMockMvc.perform(get("/api/quizzs/{id}", 99L)).andExpect(
				status().isNotFound());
	}
	...
	public void deleteQuizz() throws Exception {
		...
		// Get the quizz
		restQuizzMockMvc.perform(
				delete("/api/quizzs/{id}", quizz.getId()).accept(
						TestUtil.APPLICATION_JSON_UTF8)).andExpect(
				status().isOk());

		// Validate the database is empty
		List<Quizz> quizzs = quizzRepository.findAll();
		assertThat(quizzs).hasSize(NUMBER_OF_INITIAL_QUIZZ);
	}
}
